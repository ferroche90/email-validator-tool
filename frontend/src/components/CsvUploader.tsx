...
const CsvUploader = ({ embedded = false, onEmailsLoaded }: Props) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse<string[]>(file, {
      ...
      complete: async (resultsParsed: any) => {
        ...
        const startIndex =
          flattened.length > 0 && !flattened[0].includes('@') ? 1 : 0;
  ...
  return (
    <Stack
      ...
      sx={
        !embedded
          ? { p: 4, bgcolor: 'background.paper', borderRadius: 1, boxShadow: 1 }
          : {}
      }
    >
      {!embedded && (
      ...
      {isFileLoaded && !isParsing && (
        <Typography variant="body2" color="text.secondary">
          {t('common:fileUpload.fileLoaded').replace(
            '{{count}}',
            String(loadedEmails.length)
          )}
        </Typography>
      )}
