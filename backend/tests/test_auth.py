...
class TestSignup:
    def test_signup_success(self, client: TestClient, session: Session):
        ...
        user = session.exec("SELECT * FROM user WHERE email = 'test@example.com'").first()
        assert user is not None

        # Verify organization was created
        org = session.exec("SELECT * FROM organization WHERE slug = 'test-corp'").first()
    ...
    def test_signup_duplicate_email(self, client: TestClient, session: Session):
        ...
        user = User(
            email="existing@example.com", hashed_password="hashed_password", first_name="Existing", last_name="User"
        )
        session.add(user)
        session.commit()
    ...
    def test_signup_duplicate_organization_slug(self, client: TestClient, session: Session):
        ...
        org = Organization(name="Existing Corp", slug="existing-corp")
        session.add(org)
        session.commit()
...
class TestUserModel:
    def test_password_hashing(self):
        ...
        user = User(email="test@example.com", hashed_password=hashed, first_name="John", last_name="Doe")
